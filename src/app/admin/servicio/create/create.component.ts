...
export class CreateServicioComponent implements OnInit {
  ...
  productos: any[];
  id: number;
  constructor(private _router: Router, private _route: ActivatedRoute, private fb: FormBuilder, private _service: ServicioService) {
  ...
  selectProducto(event) {
    if (parseInt(event.data.TasaDesc, 0) > 100) {
  ...
  regresar() {
    this._router.navigate(['//servicios'], { queryParams: {} });
  }
